...
export default class TourService {
  ...
  static async getTourById(id) {
    try {
      ...
      let response = await axios.get(`http://localhost:5000/api/tour/id/${id}`)
  ...
  static async getTourByName(searchInput) {
    try {

      let response = await axios.get('http://localhost:5000/api/tours/searched', {params: {searchInput: searchInput}})
